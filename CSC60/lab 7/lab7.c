		...
		stats_t *race_stats )             /* in & out */
{
	...
	for(i=0;i< NRACERS;i++)
	{
		...
		for(j=0;j<TRIES;j++)
		{
			...
			}else if( driver_list[i].tries[j] > race_stats->slow_time)
			{
...
{
	race_stats->median=0;
	int middle_point= NRACERS/2;
	if((NRACERS % 2)==1)
	{
		race_stats->median= driver_list[middle_point].best_time;
	}else
	{
		race_stats->median=(driver_list[middle_point].best_time+driver_list[middle_point-1].best_time)/2;
	}
}
